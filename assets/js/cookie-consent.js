...
class CookieConsent {
  ...
  loadConsentData() {
    ...
    this.loadScripts(); // Load scripts after data is loaded
  }
  ...
  getItemCategory(item) {
    ...
    if (['twitter.js', 'facebook-social.js', 'youtube-embeds.js'].includes(item.script_file)) return 'social';
    return 'other';
  }

  updateUI() {
    // Update banner UI
    const categoryToggles = this.banner.querySelectorAll('.consent-category__toggle');
    const itemToggles = this.banner.querySelectorAll('.consent-item__toggle');
    ...
    itemToggles.forEach(toggle => {
      ...
      const enabled = this.preferences[scriptFile] || false;
      toggle.checked = enabled;
    });

    // Update settings modal UI
    const settingsCategoryToggles = this.modal.querySelectorAll('.settings-category__toggle');
    const settingsItemToggles = this.modal.querySelectorAll('.settings-item__toggle');
  ...
  bindEvents() {
    ...
    document.querySelectorAll('.consent-category__toggle, .settings-category__toggle').forEach(toggle => {
    ...
    // Item toggle events
    document.querySelectorAll('.consent-item__toggle, .settings-item__toggle').forEach(toggle => {
      toggle.addEventListener('change', (e) => {
    ...
    // Category header click events
    document.querySelectorAll('.consent-category__header, .settings-category__header').forEach(header => {
